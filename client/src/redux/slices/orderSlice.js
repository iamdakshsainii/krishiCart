      ...
      .addCase(updateOrderStatus.fulfilled, (state, action) => {
        ...
        if (state.farmerOrders.length > 0) {
          state.farmerOrders = state.farmerOrders.map((order) =>
            order._id === (action.payload.data?._id || action.payload._id) ? (action.payload.data || action.payload) : order
          );
        }
        ...
        toast.success(`Order status updated to ${(action.payload.data?.status || action.payload.status)}`);
      })
