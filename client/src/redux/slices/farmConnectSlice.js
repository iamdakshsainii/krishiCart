...
export const likePost = createAsyncThunk(
  ...
  async (postId, { getState, rejectWithValue, dispatch }) => {
    try {
      ...
      dispatch(
        toggleLikeOptimistic({ postId, userId: getState().auth.user._id })
      );
...
export const likeStory = createAsyncThunk(
  ...
  async (storyId, { getState, rejectWithValue, dispatch }) => {
    try {
      ...
      dispatch(
        toggleLikeStoryOptimistic({ storyId, userId: getState().auth.user._id })
      );
    ...
    try {
      ...
      dispatch(
        removeOptimisticComment({
          postId,
          commentId: `temp-comment-${Date.now()}`
        })
      );
...
export const deletePost = createAsyncThunk(
  'farmConnect/deletePost',
  async (postId, { getState, rejectWithValue, dispatch }) => {
    try {
      dispatch(removePostOptimistic(postId));
      await createApiCall(async () => {
        await axios.delete(
          `${API_URL}/farm-connect/posts/${postId}`,
          getAuthConfig(getState)
        );
      });
      return postId;
    } catch (error) {
      dispatch(restorePostOptimistic(postId));
      return rejectWithValue(handleApiError(error, 'Failed to delete post'));
    }
  }
);
export const deleteStory = createAsyncThunk(
  'farmConnect/deleteStory',
  async (storyId, { getState, rejectWithValue, dispatch }) => {
    try {
      dispatch(removeStoryOptimistic(storyId));
      await createApiCall(async () => {
        await axios.delete(
          `${API_URL}/farm-connect/stories/${storyId}`,
          getAuthConfig(getState)
        );
      });
      return storyId;
    } catch (error) {
      dispatch(restoreStoryOptimistic(storyId));
      return rejectWithValue(handleApiError(error, 'Failed to delete story'));
    }
  }
);
  ...
  async (farmerId, { getState, rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(
        handleApiError(error, 'Failed to fetch farmer profile')
      );
    }
  }
);
...
const initialState = {
  ...
  errors: {},
  profileCache: {},
  lastFetched: { posts: null, stories: null },
  ...
  reducers: {
    ...
    toggleLikeStoryOptimistic(state, action) {
      ...
      if (story) {
        ...
        const index = story.likes.findIndex(like =>
          typeof like === 'string' ? like === userId : like.user === userId
        );
        if (index >= 0) {
    ...
    removePostOptimistic(state, action) {
      ...
      if (postIndex >= 0) {
        // Store the deleted post for potential restoration
        state.deletedPosts.push(state.posts[postIndex]);
        state.posts.splice(postIndex, 1);
      }
    },
    restorePostOptimistic(state, action) {
      const postId = action.payload;
      const deletedPostIndex = state.deletedPosts.findIndex(p => p._id === postId);
      if (deletedPostIndex >= 0) {
        // Restore the post to its original position (or add to beginning)
        const restoredPost = state.deletedPosts[deletedPostIndex];
        state.posts.unshift(restoredPost);
        state.deletedPosts.splice(deletedPostIndex, 1);
      }
    },
    removeStoryOptimistic(state, action) {
      const storyId = action.payload;
      const storyIndex = state.stories.findIndex(s => s._id === storyId);
      if (storyIndex >= 0) {
        // Store the deleted story for potential restoration
        state.deletedStories.push(state.stories[storyIndex]);
        state.stories.splice(storyIndex, 1);
      }
    },
    restoreStoryOptimistic(state, action) {
      const storyId = action.payload;
      const deletedStoryIndex = state.deletedStories.findIndex(s => s._id === storyId);
      if (deletedStoryIndex >= 0) {
        // Restore the story to its original position (or add to beginning)
        const restoredStory = state.deletedStories[deletedStoryIndex];
        state.stories.unshift(restoredStory);
        state.deletedStories.splice(deletedStoryIndex, 1);
      }
    },
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(addComment.pending, (state, action) => {
        state.commentLoading[action.meta.arg.postId] = true;
      })
      ...
      .addCase(deletePost.fulfilled, (state, action) => {
        const postId = action.payload;
        state.deleteLoading[postId] = false;
        state.deletedPosts = state.deletedPosts.filter(p => p._id !== postId);
      })
      ...
      .addCase(deleteStory.fulfilled, (state, action) => {
        const storyId = action.payload;
        state.deleteLoading[storyId] = false;
        state.deletedStories = state.deletedStories.filter(s => s._id !== storyId);
      })
...
export const {
  ...
  restorePostOptimistic,
  removeStoryOptimistic,
  restoreStoryOptimistic,
