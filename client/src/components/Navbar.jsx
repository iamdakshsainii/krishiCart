              ...
              <li key={item.name}>
                ...
                >
                  ...
                  {(isDisabled || hasNoAccess) &&
                    item.name === "Farm Connect" &&
                    isMenuOpen && (
                      <FaLock className="text-xs text-gray-400 ml-auto" />
                    )}
                  {item.isNew && isMenuOpen && (
        ...
        {isAuthenticated ? (
            ...
            {isProfileOpen && isMenuOpen && (
              <div className="mt-3 space-y-2 bg-white rounded-lg shadow-lg border border-blue-100 p-2">
                {isAdmin() && (
                  <Link
                    to="/admin/dashboard"
                    className="flex items-center gap-2 px-3 py-2 hover:bg-blue-50 rounded-lg text-blue-800"
                  >
          ...
          isMenuOpen && (
            <div className="space-y-3">
              <Link
                to="/login"
                className="block px-4 py-3 text-blue-800 hover:bg-blue-50 rounded-lg font-semibold text-center"
              >
                🚀 Login
              </Link>
              <Link
                to="/register"
                className="block px-4 py-3 bg-gradient-to-r from-blue-600 to-blue-700 text-white rounded-lg font-semibold text-center shadow-lg"
              >
                ✨ Get Started
              </Link>
            </div>
